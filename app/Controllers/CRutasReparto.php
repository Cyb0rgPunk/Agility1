...
class CRutasReparto extends BaseController
{
    public function index($all_dates = null)
    {
        ...
        if($all_dates != 1){            
            ...
            $day = date("d");
            $date = date("Y-m-d");
            $where = ['MONTH(fecha) >=' =>$month,'YEAR(fecha) >=' =>$year, 'DAY(fecha) <=' => $day];
            ...
            $builder->groupBy('rr.ruta, rr.fecha, rr.hora_recogida');
            //$builder->orderBy('rr.ruta', '');
            $builder->orderBy('rr.hora_recogida ASC');
        ...
        return view('rutas/reparto/upload',$datos);
    }
    ...
    public function gestion_ruta_conductor($id_sub_zona = null, $hora_recogida = null, $fecha = null){
        $cargarRutas = new MRutasReparto();
        $datos['pasajeros'] = $cargarRutas->where(['hora_recogida' => $hora_recogida, 'ruta' => $id_sub_zona, 'fecha' => $fecha, 'conductor' => session('id_user')])->findAll();
        ...
        $datos['hora_recogida'] = $hora_recogida;
        $datos['fecha'] = $fecha;
        //dd($datos);      
        return view('rutas/reparto/reparto/gestion_conductor',$datos);

    }

    public function pasajeros_ruta($id_sub_zona = null, $hora_recogida = null, $fecha = null){
        //echo $fecha;
        //$cargarRutas = new MRutasReparto();
        //$datos['ruta']= $cargarRutas->where(['id_ruta' => $sub_zona])->findAll();

        $cargarRutas = new MRutasReparto();
        $datos['pasajeros'] = $cargarRutas->where(['hora_recogida' => $hora_recogida, 'ruta' => $id_sub_zona, 'fecha' => $fecha])->findAll();
        ...
        if(session('group_name') == 'admin' || session('group_name') == 'coordinador'){
            return view('rutas/reparto/reparto/asignar',$datos);        
        } 
    ...
    public function confirmar_ruta(){
        ...
        if(isset($_POST['btn_aceptar'])){
            $db = \Config\Database::connect();
            $builder = $db->table('t_rutas_reparto rr');
            $sql_query = "UPDATE t_rutas_reparto SET confirmacion = 1 WHERE ruta = ".$_POST['id_ruta']."  and conductor = ".session('id_user')." and fecha = '".$_POST['fecha']."' and hora_recogida = '".$_POST['hora_recogida']."'";
            //echo  $sql_query; exit();
        ...
        if(isset($_POST['btn_rechazar'])){
            $db = \Config\Database::connect();
            $builder = $db->table('t_rutas_reparto rr');
            $sql_query = "UPDATE t_rutas_reparto SET confirmacion = 0 WHERE ruta = ".$_POST['id_ruta']."  and conductor = ".session('id_user')." and fecha = '".$_POST['fecha']."' and hora_recogida = '".$_POST['hora_recogida']."'";            $builder = $db->query($sql_query);
        }        

        $ruta = "";

        if(!$builder){
            return redirect()->route(session('group_name').'/RutasReparto')
                    ->with('msg',[
                    'icon' => 'ban',
                    'type' => 'danger',
    ...
    public function asignar_ruta(){
        ...
        $db = \Config\Database::connect();
        $builder = $db->table('t_rutas_reparto rr');
        $sql_query = "SELECT id_ruta, fecha, numero_cc, nombre, direccion, barrio, hora_recogida, ruta as id_sub_zona FROM `t_rutas_reparto` WHERE id_ruta IN (".$_POST['pasajeros'][0].")";
        //echo $sql_query;
        $builder = $db->query($sql_query);
        ...
        $datos['hora_recogida'] = $_POST['hora_recogida'];
        $datos['fecha'] = $_POST['fecha'];
        //dd($datos);
        return view('rutas/reparto/reparto/asignar_movil',$datos);
    ...
    public function asignar_movil_ruta(){
        ...
        $db = \Config\Database::connect();
        $builder = $db->table('t_rutas_reparto rr');
        $sql_query = "UPDATE t_rutas_reparto SET movil = ".$_POST['id_movil']." , conductor = ".$_POST['id_conductor'].", confirmacion = NULL WHERE id_ruta IN(".$_POST['rutas_asignar'].");";
        $builder = $db->query($sql_query);
        $ruta = base_url(session('group_name').'/PasajerosRutaReparto/'.$_POST['id_ruta'].'/'.$_POST['hora_recogida'].'/'.date('Y-m-d', strtotime($_POST['fecha'])));
    ...
    public function iniciar_ruta_conductor($id_sub_zona = null, $hora_recogida = null, $fecha = null){
        $cargarRutas = new MRutasReparto();
        $datos['pasajeros'] = $cargarRutas->where(['hora_recogida' => $hora_recogida, 'ruta' => $id_sub_zona, 'fecha' => $fecha, 'conductor' => session('id_user')])->findAll();
        ...
        return view('rutas/reparto/reparto/iniciar_ruta',$datos);
    ...
    public function asistencia_ruta(){
        ...
        // dd($_POST['abordaron'][0]);
         ...
         $db = \Config\Database::connect();
         $builder = $db->table('t_rutas_reparto rr');
         $sql_query = "UPDATE t_rutas_reparto SET abordo = 1 WHERE id_ruta IN(".$_POST['abordaron'][0].");";
         $builder = $db->query($sql_query);
         $ruta = base_url(session('group_name').'/PasajerosRutaReparto/'.$_POST['id_ruta'].'/'.$_POST['hora_recogida'].'/'.date('Y-m-d', strtotime($_POST['fecha'])));
    ...
    public function upload(){
        ...
        if($sheetcount>1){
            for ($i=1; $i < $sheetcount; $i++) { 
                ...
                //dd($data);
                $cargarSolicud = new MRutasReparto();
